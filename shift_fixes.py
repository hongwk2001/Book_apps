...
for i, d in enumerate(data):
    if d['id'] == 15:
        ...
        en_text = d['en']
        parts = en_text.split('The damp, cold white clouds')
        data[i]['en'] = parts[0].strip()
        data[i+1]['en'] = 'The damp, cold white clouds' + parts[1] + ' ' + data[i+1]['en']
    elif d['id'] == 20:
        ko_text = d['ko']
        ...
        en_text = d['en']
        parts = en_text.split('Or rather, she might not even know')
        data[i]['en'] = parts[0].strip()
        data[i+1]['en'] = 'Or rather, she might not even know' + parts[1] + ' ' + data[i+1]['en']

    if d['id'] == 6:
        data[i]['ko'] = data[i]['ko'].replace('휘몰아쳤다. 기록에', '휘몰아쳤다, 기록에')
    elif d['id'] == 17:
        data[i]['en'] = data[i]['en'].replace('out to sea. Now the lightning', 'out to sea, and now the lightning')
    elif d['id'] == 59:
